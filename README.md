...
Depending on your operating system, you may need to install additional system dependencies. Follow the official [Tauri Setup Guide](https://v2.tauri.app/start/prerequisites/) for your platform:
...
The application expects the `spx-server` executable to be present. During development, ensure it is available in the expected path (refer to `src-tauri/src/lib.rs` for path resolution logic).
...
The following directories and files are required in the `src-tauri/` directory:
